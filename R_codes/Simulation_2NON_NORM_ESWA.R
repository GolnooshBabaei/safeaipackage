...
### EXPERIMENT 1 (NON_NORMAL DISTRIBUTION WITH CORR MATRIX 2) ###
#################################################################
# Packages
...
boxplot(data,las=2,col=c("lightskyblue1","aquamarine1","khaki1",
        ...
        at=c(1,2,3, 5,6,7, 9,10,11),par(mar=c(5,4,4,2)+0.1),main="RGA, RGR, RGF: Non-Normal Distribution - Correlation matrix S2", 
...
boxplot(data,las=2,col=c("royalblue","aquamarine1","khaki1",
        ...
        at=c(1,2,3, 5,6,7, 9,10,11, 13,14,15),par(mar=c(5,4,4,2)+0.1),main="RGE: Non-Normal Distribution - Correlation matrix S2", 
...
# RGR
#  lin reg > rand for > reg tree
pieces.RGR<-c(RGR_lin_reg,RGR_rf_mod,RGR_reg_tree) 
n<-c(1000,1000,1000) 
grp.r<-as.ordered(factor(rep(1:length(n),n))) 
...
# RGE
# Variable 1
# rand for > lin reg > reg tree
pieces.RGE_X1<-c(RGE_rf_mod_X1,RGE_lin_reg_X1,RGE_reg_tree_X1) 
n<-c(1000,1000,1000) 
grp.r<-as.ordered(factor(rep(1:length(n),n))) 
...
pieces.RGE_X3<-c(RGE_rf_mod_X3,RGE_reg_tree_X3,RGE_lin_reg_X3) 
n<-c(1000,1000,1000) 
grp.r<-as.ordered(factor(rep(1:length(n),n))) 
