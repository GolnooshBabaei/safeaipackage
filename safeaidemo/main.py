...
def dialog(c_name:str, c_desc:str, c_type:ColumnDataType):
    ...
    def _dialog():
        descriptor = ColumnDescriptorStrategy(c_name, c_desc, c_type,st.session_state.to_dict())()
        form = descriptor.form
        s1 = form.form_submit_button(
            "Submit",
            type="primary",
                            ...
                            if _this := list(rows[i:i+3]):
                                for c, j in enumerate(st.columns(len(_this))):
                                    j.button(
                                        ...
                                        on_click=dialog,
                                        args=(_this[c][1][0], _this[c][1][2], _this[c][1][1])
                                    )
                                continue
                    st.write("---")
