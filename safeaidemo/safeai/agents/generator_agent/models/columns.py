...
from safeai.utils.base import Base
...
class ColumnDescriptorFactory(Base):
    ...
    data_sample_description: str | None = Field(
        None, title="Sample Description", description="A Description of the Sample Data"
    )
...
class DateDataDescription(ColumnDescriptorFactory):
    ...
    def form(cls, c_name: str) -> DeltaGenerator:
        ...
        form.date_input("Minimum Value", key="start_date", value=date.today())
        form.date_input("Maximum Value", key="end_date", value=date.today())
        form.selectbox("Frequency", TimeFrequency.to_list(), key="frequency", index=0)
        return form
...
class NumericDataDescription(ColumnDescriptorFactory):
    ...
    min: float = Field(
        0.0, title="Minimum Value", description="The Minimum Value in the Column"
    )
    max: float = Field(
        100.0, title="Maximum Value", description="The Maximum Value in the Column"
    )
    mean: float = Field(
        50.0, title="Mean Value", description="The Mean Value in the Column"
    )
    median: float = Field(
        50.0, title="Median Value", description="The Median Value in the Column"
    )
    iqr1: float = Field(
        25.0, title="First Quartile", description="The First Quartile of the Column"
    )
    iqr3: float = Field(
        75.0, title="Third Quartile", description="The Third Quartile of the Column"
    )
    std_dev: float = Field(
        1.0,
    ...
    skewness: float = Field(
        0.0, title="Skewness", description="The Skewness of the Column"
    )
    kurtosis: float = Field(
        0.0, title="Kurtosis", description="The Kurtosis of the Column"
    )
    ...
    def form(cls, c_name: str) -> DeltaGenerator:
        ...
        for k, v in cls.model_fields.items():
            form.number_input(v.title or k, key=k, value=v.default)
        return form
...
class CategoricalDataDescription(ColumnDescriptorFactory):
    ...
    unique_values: list[str] = Field(
        ..., title="Unique Values", description="The Unique Values in the Column"
    )
    value_coverage: dict[str, int] = Field(
        ..., title="Value Counts", description="'%' of Values in the Column"
    )
...
class TextDataDescription(ColumnDescriptorFactory):
    ...
    text_type: TextType | None = Field(
        None, title="Text Type", description="The Type of the Text Domain"
    )
    ...
    emotions: list[Emotion] | None = Field(
        None, title="Emotions", description="The Emotions of the Text Domain"
    )
    ...
    def form(cls, c_name: str) -> DeltaGenerator:
        ...
        form = st.form(key=form_key)
        tt = form.selectbox(
            "Text Type", TextType.to_list(), key="text_type", index=None
        )
        ...
        form.multiselect("Emotions", Emotion.to_list(), key="emotions", default=None)
        return form
...
class ColumnDescriptorStrategy:
    ...
    def __init__(
        self, c_name: str, c_desc: str, c_type: ColumnDataType, state: dict
    ) -> None:
        self.c_name = c_name
        self.c_desc = c_desc
        self.c_type = c_type
    ...
    def __call__(self, *args, **kwds) -> DescriptorModel:
        if (
            self.c_type == ColumnDataType.DATE
            ...
            self.c_type == ColumnDataType.CATEGORICAL
            or self.c_type == ColumnDataType.ORDINAL
            or self.c_type == ColumnDataType.BOOLEAN
        ):
            return DescriptorModel(
                col_name=self.c_name,
                col_desc=self.c_desc,
            ...
            self.c_type == ColumnDataType.INTEGER
