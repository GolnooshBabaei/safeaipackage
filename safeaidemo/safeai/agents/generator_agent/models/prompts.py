...
from safeai.agents.generator_agent.models.columns import (
    ColumnDataType,
    ColumnDescriptorStrategy,
)
...
class DataTypePromptFactory:
    ...
    def __init__(self, col_name: str, col_desc: str, state: dict) -> None:
        self.data_description = ColumnDescriptorStrategy(
            col_name, col_desc, ColumnDataType.TEXT, state
        )
...
class TextPrompt(DataTypePromptFactory):
    ...
    def role_prompt(self) -> Chat:
        ...
        if self.data:
            ...
            prompt += f"""
                    Using my company's profile and data description generate ````{self.data_profile.data_num_rows}```\
                realistic real world samples for the ```{self.data_description.c_name}``` feature.\n
                """

        return Chat(role="user", content=prompt)


class NumericPrompt(DataTypePromptFactory):
    """_summary_"""

    @override
    def role_prompt(self) -> Chat:
        raise NotImplementedError("Method role_prompt not implemented")


class CategoricalPrompt(DataTypePromptFactory):
    """_summary_"""
...
class DatePrompt(DataTypePromptFactory):
    """_summary_"""
...
class BooleanPrompt(DataTypePromptFactory):
    """_summary_"""
...
class OrdinalPrompt(DataTypePromptFactory):
    """_summary_"""
...
class PromptStrategy:
    ...
    def __call__(self) -> DataTypePromptFactory:
        if self.c_type == ColumnDataType.TEXT:
            ...
            return NumericPrompt(self.c_name, self.c_desc, self.state)
        elif self.c_type == ColumnDataType.CATEGORICAL:
            return CategoricalPrompt(self.c_name, self.c_desc, self.state)
        elif self.c_type == ColumnDataType.DATE:
            return DatePrompt(self.c_name, self.c_desc, self.state)
        elif self.c_type == ColumnDataType.BOOLEAN:
            return BooleanPrompt(self.c_name, self.c_desc, self.state)
        elif self.c_type == ColumnDataType.ORDINAL:
            return OrdinalPrompt(self.c_name, self.c_desc, self.state)
