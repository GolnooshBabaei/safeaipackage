...
from safeai.enums import Country, AgentDomain, AgentModel
...
def safeai_agent():
    """_summary_"""
    st.radio("", AgentModel.to_list(), key="agent_model", index=0)
    cols = st.columns(2)
    with cols[0]:
        st.date_input("Context Start", key="agent_domain_start", value=date.today())
    with cols[1]:
        st.date_input(
            "Context End",
...
def experiment():
    """_summary_"""
    st.slider(
        "Number of Trials", min_value=1, max_value=1000, key="exp_trials", value=100
    )
    st.multiselect(
        "Evaluate On:",
